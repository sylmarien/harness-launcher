# Using harness-launcher

How to run the app and use it well. [`CHEATSHEET.md`](../../CHEATSHEET.md)
lists every key and mark on one page.
[`docs/developers/`](../developers/README.md) covers how the app is built.

- [Getting started](getting-started.md) — from install to a running spawn in
  about a minute.
- [The command line](the-cli.md) — invocation, `--and`, models and effort
  levels.
- [The spawn form](the-spawn-form.md) — composing work inside the app.
- [What it supports](what-it-supports.md) — what the app does and does not
  do, and its edge cases.
